...
<div class="wrapper wrapper-content animated fadeInUp">
    ...
    <div class="ibox">
        ...
        <div class="ibox-content">
            ...
            <h4>Хуудасны дэвсгэр — зураг / видео</h4>
            <p class="text-muted" style="margin-top:-4px">
                Энэ зураг эсвэл видео <b>БҮХ хуудсанд тогтмол</b> зогсоно. Доошоо гүйлгэхэд дэвсгэр нь
                байрандаа үлдэж, зөвхөн текст нь дээгүүр нь хөвж өнгөрнө.
                Хуудсан дээрээ шууд засварлах горимд "Дэвсгэр" товчоор ч солино.
            </p>
            <div class="row">
                <div class="col-sm-6">
                    <div class="form-group">
                        <label class="font-noraml">Дэвсгэр зураг</label>
                        <div class="input-group">
                            <input type="text" class="form-control input-sm" id="regPageBgPic" name="frmSet[pageBgPic]"
                                   value="<?php echo RegistrationCore::esc($regSet["pageBgPic"]);?>" placeholder="/postpic/image/...">
                            <span class="input-group-btn">
                                <button class="btn btn-default btn-sm" type="button" onclick="regOpenPicker('regPageBgPic')">Сонгох</button>
                            </span>
                        </div>
                        <?php if (trim($regSet["pageBgPic"]) != "") { ?>
                        <div style="margin-top:6px">
                            <img src="<?php echo RegistrationCore::esc(newsPicFnc(0, $regSet["pageBgPic"]));?>" alt="" style="max-height:80px;border:1px solid #e5e5e5">
                        </div>
                        <?php } ?>
                    </div>
                </div>
                <div class="col-sm-6">
                    <div class="form-group">
                        <label class="font-noraml">Дэвсгэр видео (mp4 / webm)</label>
                        <div class="input-group">
                            <input type="text" class="form-control input-sm" id="regPageBgVideo" name="frmSet[pageBgVideo]"
                                   value="<?php echo RegistrationCore::esc($regSet["pageBgVideo"]);?>" placeholder="/postpic/image/...">
                            <span class="input-group-btn">
                                <button class="btn btn-default btn-sm" type="button" onclick="regOpenPicker('regPageBgVideo')">Сонгох</button>
                            </span>
                        </div>
                        <small class="text-muted">Видео тавибал зургийн оронд түүнийг тоглуулна.</small>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-sm-3">
                    <div class="form-group">
                        <label class="font-noraml">Бараан бүрхүүл (%)</label>
                        <input type="number" min="0" max="100" class="form-control input-sm" name="frmSet[pageBgOverlay]"
                               value="<?php echo (int)$regSet["pageBgOverlay"];?>">
                        <small class="text-muted">Текст уншигдахуйц болгоно.</small>
                    </div>
                </div>
                <div class="col-sm-3">
                    <div class="form-group">
                        <label class="font-noraml">Бүдгэрүүлэх (px)</label>
                        <input type="number" min="0" max="30" class="form-control input-sm" name="frmSet[pageBgBlur]"
                               value="<?php echo (int)$regSet["pageBgBlur"];?>">
                    </div>
                </div>
                <div class="col-sm-3">
                    <div class="form-group">
                        <label class="font-noraml">Зургийн байрлал</label>
                        <select class="form-control input-sm" name="frmSet[pageBgPos]">
                            <?php foreach (array("center" => "Гол", "top" => "Дээд", "bottom" => "Доод") as $pk => $pl) { ?>
                            <option value="<?php echo $pk;?>" <?php if($regSet["pageBgPos"]==$pk) echo "selected";?>><?php echo $pl;?></option>
                            <?php } ?>
                        </select>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-sm-3">
                    <div class="form-group">
                        <label class="font-noraml">Хэсэг бүр дэлгэц дүүрэн</label>
                        <select class="form-control input-sm" name="frmSet[scrollSnap]">
                            <option value="1" <?php if($regSet["scrollSnap"]=="1") echo "selected";?>>Тийм (snap)</option>
                            <option value="0" <?php if($regSet["scrollSnap"]!="1") echo "selected";?>>Үгүй (энгийн)</option>
                        </select>
                    </div>
                </div>
                <div class="col-sm-3">
                    <div class="form-group">
                        <label class="font-noraml">Баруун талын цэгэн цэс</label>
                        <select class="form-control input-sm" name="frmSet[scrollDots]">
                            <option value="1" <?php if($regSet["scrollDots"]=="1") echo "selected";?>>Харуулах</option>
                            <option value="0" <?php if($regSet["scrollDots"]!="1") echo "selected";?>>Нуух</option>
                        </select>
                    </div>
                </div>
            </div>
