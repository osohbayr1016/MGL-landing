<!DOCTYPE html>
<html lang="<?php echo SeoMeta::esc(isset($gloLangObj["langKey"]) && $gloLangObj["langKey"]!="" ? strtolower($gloLangObj["langKey"]) : "mn");?>">
<head>
    <meta http-equiv="X-UA-Compatible" content="IE=edge" />
    <meta charset="utf-8" />
<?php
	/* Title / description / canonical / Open Graph / Twitter card / JSON-LD.
	   Сайтын өгөгдмөл: site.info.php ($gloMeta). Мэдээний дэлгэрэнгүй:
	   widgets/newsmore/sys.php дарж бичнэ. Бусад хуудас $addPageTitle /
	   $siteInfoDes / $siteInfoImg-ээ тавьсан бол энд тусгагдана. */
	if(!empty($addPageTitle) && $gloMeta["type"]!="article")
		$gloMeta["title"] = SeoMeta::plainText($addPageTitle, 120);

	if($gloMeta["type"]!="article"){
		if(isset($siteInfoDes) && SeoMeta::plainText($siteInfoDes)!="")
			$gloMeta["description"] = SeoMeta::plainText($siteInfoDes, 300);

		if(!empty($siteInfoImg) && $siteInfoImg!=SeoMeta::absUrl(SeoMeta::defaultImagePath())){
			$pageImg = SeoMeta::resolveImage($siteInfoImg);
			$gloMeta["image"]       = $pageImg["url"];
			$gloMeta["imageWidth"]  = $pageImg["width"];
			$gloMeta["imageHeight"] = $pageImg["height"];
			$gloMeta["imageType"]   = $pageImg["type"];
			$gloMeta["imageAlt"]    = $gloMeta["title"];
		}
	}

	echo SeoMeta::render($gloMeta);
?>
...
<link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@100;200;300;500;600;700;800&display=swap" rel="stylesheet">
    ...
    <link href="/assets/css/partners.css?v=<?php echo time()?>" rel="stylesheet">
    <link href="/assets/css/erp-login.css?v=<?php echo time()?>" rel="stylesheet">
    <?php if(!empty($applyCssOn)) { ?>
    <link href="/assets/css/apply.css?v=<?php echo time()?>" rel="stylesheet">
    <?php } ?>
...
<body class="<?php echo $clkMenuObj["pageType"]." ".$pageHeaderCls;?>">
   ...
   <?php include $incPageUrl; ?> 
    ...
    <script src="/assets/js/all.js?v=5"></script>
    <script src="/assets/js/mobile.js?v=2"></script>

    <?php include "erp-login.php"; ?>
    <script src="/assets/js/erp-login.js?v=1"></script>
	<?php 
if(isset($widJsArr))
if(count($widJsArr)>0)
